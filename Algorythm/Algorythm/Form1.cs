        ...
        public void FindWSavOptimal(bool intersection, int[] WOstrategies, int[] SavOstrategies , int[] WSavIntersection, int count_conditions, double[,] matrix, double Sav_s, double W_s, double[] Sav_i, double[] W_i) {
            ...
            if (intersection) {
                ...
                if ((!true_for_everyone) || (NoWorSavStrategies.Length == 0)) { 
                    ...
                    str = "Структура множества стратегий, оптимальных по критерию Вальда-Сэвиджа имеет вид: ";
                    listBox3.Items.Add(str);
...

            {
                ...
                int[] PlacesINeachA;
                string places;
                for (int i = 0; i < PlacesINeachAlpha.GetLength(0) - 1; i+=2) 
                {
                ...
                dataGridView8.Rows.Add();
                dataGridView8[0, PlacesINeachAlpha.GetLength(0) - 1].Value = Math.Round(intersections[intersections.Length - 1], 6);
                for (int i = 1; i < PlacesINeachAlpha.GetLength(1) + 1; i++)
                {
                    ...
                    dataGridView8[i, PlacesINeachAlpha.GetLength(0) - 1].Value = places;
                }
